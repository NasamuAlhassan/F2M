...
const EVENT_STYLE: Record<string, { dot: string; label: string }> = {
  ...
  TRANSPORT_REQUESTED: { dot: 'bg-sky-500', label: 'Transport requested' },
  DRIVER_ASSIGNED: { dot: 'bg-sky-600', label: 'Driver assigned' },
  TRANSPORT_FUNDED: { dot: 'bg-indigo-600', label: 'Transport fee held in escrow' },
  IN_TRANSIT: { dot: 'bg-violet-600', label: 'In transit — driver picked up' },
  TRANSPORT_DELIVERED: { dot: 'bg-teal-600', label: 'Delivered — buyer confirmed receipt' },
  DRIVER_PAID: { dot: 'bg-green-600', label: 'Driver paid' },
  TRANSPORT_CANCELLED: { dot: 'bg-red-500', label: 'Transport cancelled' },
  VOICE_CALL: { dot: 'bg-stone-500', label: 'Voice call' },
};
